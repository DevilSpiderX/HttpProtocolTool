package DevilSpiderX.httpProtocolTool.lang;

import DevilSpiderX.httpProtocolTool.exception.BytesIndexOutOfBoundsException;
...
public class Bytes implements java.io.Serializable, Comparable<Bytes>, ByteSequence {
    ...
    /**
     * 通过使用指定字符集{@linkplain java.nio.charset.Charset charset}编码指定的字符串来构造新的字节串。
     * 新的<code>Bytes</code>的长度是字符集的函数，因此可能不等于字符串的长度。
     * 指定字节在默认字符集中无效时，此构造函数的行为是未指定的。
     * 当需要对编码过程进行更多的控制时，应使用{@link java.nio.charset.CharsetEncoder}类。
     *
     * @param string  要编码的字符串
     * @param charset 该{@linkplain java.nio.charset.Charset charset}被用于编码字符串
     */
...
//                }
     ...
     * 当需要对解码过程进行更多控制时，应使用{@link java.nio.charset.CharsetDecoder}类。
     *
     * @param charsetName 支持的{@linkplain java.nio.charset.Charset charset}名称
     * @return 结果字符串
     * @throws UnsupportedEncodingException 如果是不支持的字符集的名称
     */
    ...
    /**
     * 使用给定的{@linkplain java.nio.charset.Charset charset}将该<code>Bytes</code>解码为一个字符串，将结果存储到新的字符串中。
     * 此方法总是用此字符集的默认替换字符串替换格式错误的输入和不可映射字节序列。
     * 当需要对解码过程的更多控制时，应使用{@link java.nio.charset.CharsetDecoder}类。
     *
     * @param charset 该{@linkplain java.nio.charset.Charset charset}被用于解码Bytes
     * @return 结果字符串
     */
